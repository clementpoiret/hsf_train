...
class TorchToONNX(BaseExporter):
        ...
        **export_kwargs,
        ...
        super().__init__(
            [
                ...
                _TorchOnnxExport(sample_batch, opset, disable_bn_fusing, **
                                 export_kwargs),
    ...
    def export(self, pre_transforms_model: torch.nn.Module, file_path: str):
        post_transforms_model: onnx.ModelProto = self.apply(
...
class _TorchOnnxExport(BaseTransform):
    ...
    def transform(self,
                  ...
                  file_name: str = "model.onnx") -> onnx.ModelProto:
        ...
        for submodule in module.modules():
            if (hasattr(submodule, "observer_enabled") and
        ...
        is_quant_module = any(
            ...
            for submodule in module.modules())
        if (_PARSED_TORCH_VERSION >= version.parse("1.7") and
                not is_quant_module and self.disable_bn_fusing):
...
def _wrap_batch_norms(module: torch.nn.Module) -> bool:
    ...
    for name, submodule in module.named_modules():
        if (isinstance(submodule, torch.nn.BatchNorm1d) or
                isinstance(submodule, torch.nn.BatchNorm2d) or
                isinstance(submodule, torch.nn.BatchNorm3d)):
            submodule_path = name.split(".")
            parent_module = _get_submodule(module, submodule_path[:-1])
            setattr(parent_module, submodule_path[-1],
                    _AddNoOpWrapper(submodule))
